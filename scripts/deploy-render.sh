...
git add .
git commit -m "Deploy: Fix ERESOLVE com --legacy-peer-deps $(date '+%Y-%m-%d %H:%M:%S')" || echo "Nenhuma mudança para commitar"
...
echo "   - Frontend: raiz do projeto (com --legacy-peer-deps)"
...
echo "   - lovable-tagger vs vite conflict resolvido temporariamente"
...
echo ""
echo "🔴 AÇÃO NECESSÁRIA LOCALMENTE:"
echo "   1. Execute: npm install (para criar package-lock.json)"
echo "   2. Faça commit do package-lock.json gerado"
echo "   3. Para correção definitiva: altere vite para ^5.4.19 no package.json"
